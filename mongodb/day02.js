...
db.member.insertMany([
	...
	{name:'이신혜',userid:'leeshinhye',tel:'010-6222-3333',age:23}	
]);
db.member.find()
...
-------------------------------------------------------------
*/
//나이가 20세이거나 아이디가 'hong2'인 회원정보를 보여주세요
db.member.find({$or:[{age:20},{userid:'hong2'}]})
//select * from member where age=20 or userid='hong2'
...
db.member.find({$and:[{age:25},{name:'김유신'}]})
db.member.find({age:25,name:'김유신'})
...
//$exists:true, true=>1, false=>0으로 대체가능
db.member.find({grade:{$exists:false}}).count()
...
//updateMany(): 여러개 수정
//replaceOne({조건},{교체할문서}): 도큐먼터를 교체하는 함수
//replaceOne()의 옵션 upsert:true값을 주면. 검색한 조건의 문서가 있으면 교체하고, 없다면 insert함
db.member.find()
db.member.replaceOne({userid:/lee/},{name:'이해신',userid:'lee',tel:'010-6222-3333',age:24})
db.member.replaceOne({userid:'choi'},{name:'최명길',userid:'choi',tel:'010-7222-3333',age:25},{upsert:true})

//updateOne()/updateMany()
//$set 연산자 : 필드값을 설정할때 사용
db.member.updateMany({age:{$lt:22}},{$set:{grade:'B+'}})
db.member.find()



//tags "red","blank" 값이 있는 상품을 조회
...
//tags배열의 크기가 2인 상품 가져오세요
db.inventory.find({tags:{$size:2}})

//dim_cm 에 25이상 값이 포함된 상품을 가져오세요
