...
//select * from articles where athor='annT'
db.articles.aggregate([{
    $match:{author:'li'}
}])
...
//2개 이상의 필드로 group할때는 _id:{key:'$필드명',key:'$필드명'}
...
db.inventory.find()
//하나의 도큐번트를 sizes배열 요소로 분리해서 각각의 도큐먼트로 보여준다
...
//select cust_id, sum(a.qty) totalQty
//from orders o join orders_item a
//on o.onum = a.onum group by cust_id
db.orders.aggregate([
{
    $unwind:'$items'
}
])
//고객별 주문 아이템들의 총수량을 보여주세요
db.orders.aggregate([
{
    $unwind:'$items'
},
...
//select count(*) from (select cust_id, ord_date from orders group by cust_id, ord_date) as d
db.orders.aggregate([
{
...
//items에서 날짜별 총구매가격(price * quantity), 평균수랑, 주문건수(count)를 구해 보여주세요
