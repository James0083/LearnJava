...
--------------------------------------------
--사번을 인 파라미터로 넘기면 해당 사원의 부서번호,사원명,부서명을 출력하는 프로시저
...
begin
    ...
    if vcomm is null then ysal := vsal*12;
    else ysal := vsal*12+vcomm;
    end if;
    dbms_output.put_line(pname||'님의 연봉은 '||to_char(ysal,'$999,999')||'입니다.');
...
/
-------------------------------------------
--for 루프문을 이용하면 cursor를 open, fetch, close 절차가 필요 없다.
--자동으로 알아서 함.
declare
begin
...
begin
    loop
        ...
        values (veno, upper('James'), sysdate);
        veno := veno+10;
        exit when veno >= 9090;
    end loop;
    dbms_output.put_line('데이터 삽입 성공 '||(vno-9000)/10||'건의 데이터 입력함');
end;
/
...
BEGIN
    ...
    LOOP
        FETCH DCR INTO VDNAME, VCNT, VAVG_SAL;
        EXIT WHEN DCR%NOTFOUND;
        SELECT DEPTNO INTO VDNO
        FROM DEPT WHERE DNAME=VDNAME;
        
        DBMS_OUTPUT.PUT_LINE(VDNO||'    '||RPAD(VDNAME, 12, ' ')||' '||VCNT||CHR(9)||VAVG_SAL);
    END LOOP;
END;
/
...
/*
    ...
    WHEN OTHERS THEN
        ...
        -- SQLERRM : 에러메세지, SQLCODE : 에러코드 
END;
/
EXEC EXCEPT_TEST(10, 1002, 'PETER');
EXEC EXCEPT_TEST(10, 1001, 'PETER');
